...
const manifestUrl = '/static/showcase2/bronze-chariot/manifest.json';
...
class Showcase2App {
    private readonly canvas = document.getElementById('showcase2-canvas') as HTMLCanvasElement;
    private readonly loader = document.getElementById('showcase2-loader') as HTMLDivElement;
    private readonly loaderText = document.getElementById('showcase2-loader-text') as HTMLParagraphElement;
    private readonly titleText = document.getElementById('showcase2-title-text') as HTMLHeadingElement;
    private readonly subtitleText = document.getElementById('showcase2-subtitle-text') as HTMLParagraphElement;
    private readonly debugLog = document.getElementById('showcase2-debug-log') as HTMLDivElement;
    private readonly debugStatus = document.getElementById('showcase2-debug-status') as HTMLSpanElement;
    ...
    async boot() {
        this.log('info', 'Showcase2 GLB boot start');
        this.setLoading('Loading Showcase2 manifest...');
        const manifest = await fetch(manifestUrl).then((res) => res.json()) as Manifest;
        this.titleText.textContent = manifest.title;
        ...
        this.app.scene.ambientLuminance = 1600;
        ...
        this.modelRoot = new Entity('showcase2-model-root');
        this.app.root.addChild(this.modelRoot);

        this.cameraEntity = new Entity('showcase2-camera');
        ...
        this.addDirectional('key', { ...manifest.presentation.lighting.key, intensity: 3.2 });
        this.addDirectional('fill', { ...manifest.presentation.lighting.fill, intensity: 1.4 });
        this.addDirectional('rim', { ...manifest.presentation.lighting.rim, intensity: 1.1 });
    ...
    private log(level: 'info' | 'warn' | 'error', message: string) {
        ...
        line.className = `showcase2-log-line showcase2-log-${level}`;
        const time = document.createElement('span');
        time.className = 'showcase2-log-time';
    ...
    private addDirectional(name: string, preset: Manifest['presentation']['lighting']['key']) {
        const entity = new Entity(`showcase2-${name}`);
    ...
    private async installEnvironment(url: string) {
        ...
        const texture = new Texture(this.app.graphicsDevice, {
            name: 'showcase2-env', projection: TEXTUREPROJECTION_EQUIRECT, mipmaps: true,
...
new Showcase2App().boot().catch((error) => {
    console.error(error);
    const loader = document.getElementById('showcase2-loader-text');
    const status = document.getElementById('showcase2-debug-status');
    const log = document.getElementById('showcase2-debug-log');
    const message = error instanceof Error ? error.message : String(error);
    if (loader) loader.textContent = message;
    if (status) status.textContent = message;
    if (log) {
        const line = document.createElement('div');
        line.className = 'showcase2-log-line showcase2-log-error';
