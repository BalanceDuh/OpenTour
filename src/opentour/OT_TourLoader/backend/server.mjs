...
const server = createServer(async (req, res) => {
    try {
        ...
        if (path === '/api/ot-tour-loader/health' && req.method === 'GET') {
            json(res, 200, { ok: true, service: 'ot-tour-loader', version: '1.0.0' });
            return;
        }
        ...
        if (path === '/api/ot-tour-loader/state' && req.method === 'PUT') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
            if (!modelFilename) return fail(res, 400, 'OT_TL_VALIDATION_ERROR', 'modelFilename required');
            const pois = Array.isArray(body.pois) ? body.pois : [];
            const updatedAt = saveState(modelFilename, body.profile || {}, pois);
            json(res, 200, { ok: true, modelFilename, totalPois: pois.length, updatedAt });
            return;
        }
        ...
        if (path === '/api/ot-tour-loader/llm-config' && req.method === 'GET') {
            ...
            json(res, 200, {
                ...
                modelFilename,
                llm: {
                    selectedProvider: globalCfg.selectedProvider,
                    gemini: globalCfg.gemini,
                    qwen: globalCfg.qwen,
                    promptTemplate: promptRow?.prompt_template || DEFAULT_PROMPT_TEMPLATE,
                    csvPromptTemplate: promptRow?.csv_prompt_template || DEFAULT_CSV_PROMPT_TEMPLATE,
                    movePromptTemplate: promptRow?.move_prompt_template || DEFAULT_MOVE_PROMPT_TEMPLATE,
                    updatedAt: globalCfg.updatedAt,
                    promptUpdatedAt: promptRow?.updated_at || null,
                    scope: 'global-key-model + model-prompts'
                }
            });
            return;
        }

        if (path === '/api/ot-tour-loader/llm-config' && req.method === 'PUT') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
            if (!modelFilename) return fail(res, 400, 'OT_TL_VALIDATION_ERROR', 'modelFilename required');
            const now = new Date().toISOString();
            ...
            upsertPromptConfigStmt.run({
                ...
                move_prompt_template: String(body?.llm?.movePromptTemplate || DEFAULT_MOVE_PROMPT_TEMPLATE),
                updated_at: now
            });
            json(res, 200, { ok: true, modelFilename, updatedAt: now });
            return;
        }

        if (path === '/api/ot-tour-loader/llm/direct-call' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
        ...
        if (path === '/api/ot-tour-loader/pois' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
            if (!modelFilename) return fail(res, 400, 'OT_TL_VALIDATION_ERROR', 'modelFilename required');
            const p = body.poi || {};
        ...
        const patchMatch = getPathMatch(path, '/api/ot-tour-loader/pois/:poiId');
        ...
        const shotMatch = getPathMatch(path, '/api/ot-tour-loader/pois/:poiId/screenshot');
        ...
        if (path === '/api/ot-tour-loader/csv/export/jobs' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
            if (!modelFilename) return fail(res, 400, 'OT_TL_VALIDATION_ERROR', 'modelFilename required');
        ...
        if (path === '/api/ot-tour-loader/csv/versions/generate' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
            if (!modelFilename) return fail(res, 400, 'OT_TL_VALIDATION_ERROR', 'modelFilename required');
            const globalLlmCfg = resolveGlobalLlmConfig(getLlmConfigStmt.get(GLOBAL_LLM_CONFIG_KEY));
        ...
        if (path === '/api/ot-tour-loader/csv/timing/estimate' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body?.modelFilename || '').trim();
        ...
        const csvExportEventsMatch = getPathMatch(path, '/api/ot-tour-loader/csv/export/jobs/:jobId/events');
        ...
        const csvExportDownloadMatch = getPathMatch(path, '/api/ot-tour-loader/csv/export/jobs/:jobId/download');
        ...
        const csvVersionMatch = getPathMatch(path, '/api/ot-tour-loader/csv/versions/:versionId');
        ...
        const csvVersionSaveAsNewMatch = getPathMatch(path, '/api/ot-tour-loader/csv/versions/:versionId/save-as-new');
        ...
        const csvVersionConfirmMatch = getPathMatch(path, '/api/ot-tour-loader/csv/versions/:versionId/confirm');
        ...
        const csvVersionDownloadMatch = getPathMatch(path, '/api/ot-tour-loader/csv/versions/:versionId/download');
        ...
        if (path === '/api/ot-tour-loader/csv/import' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
        ...
        if (path === '/api/ot-tour-loader/content/jobs' && req.method === 'POST') {
            const body = JSON.parse(await readBody(req) || '{}');
            const modelFilename = String(body.modelFilename || '').trim();
        ...
        const streamMatch = getPathMatch(path, '/api/ot-tour-loader/content/jobs/:jobId/events');
        ...
        const stopMatch = getPathMatch(path, '/api/ot-tour-loader/content/jobs/:jobId/stop');
        ...
        const resumeMatch = getPathMatch(path, '/api/ot-tour-loader/content/jobs/:jobId/resume');
...
const port = Number(process.env.OT_TOUR_LOADER_PORT || 3031);
server.listen(port, () => {
    console.log(`[ot-tour-loader] listening on http://localhost:${port}`);
    console.log(`[ot-tour-loader] llm dispatcher: ${LLM_PROXY_URL ? `proxy ${LLM_PROXY_URL}` : 'direct'}`);
});
